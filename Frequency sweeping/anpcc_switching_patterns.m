function [Pc_comp,Ps_comp,Ptot_comp,Ptot]=anpcc_switching_patterns(U__DC,Vac,Sac,ddeg,f__s,t__a,U__s,L__s)

d=ddeg/180.*pi;

Iac_rms=abs(Sac/(sqrt(3)*Vac)); %% RMS AC current
...
% L__s=1; %%% time p.u. on sequence N <--> 0L1 / N <--> 0L2 is (1-L__s) % i think here it is P
...
irms_d56=sqrt(3)*sqrt(-2*m*(t__a+t__b)*cos(d)^2-(4*(m-3*sin(d)*(1/4)))*(t__a-t__b)*cos(d)+...
    (-3*d-2*m+3*pi)*t__a+3*t__b*(d-2*m*(1/3)))*abs(I__ac)/(6*sqrt(pi));
...
Ps_d56=t__a*f__s*U__DC*(b__2d*I__ac^2*cos(d)*sin(d)+b__2d*I__ac^2*pi-b__2d*I__ac^2*d+...
    2*b__1d*I__ac*cos(d)+2*b__1d*I__ac+2*b__0d*pi-2*b__0d*d)/(8*pi*U__Bd);

% -t__a.*f__s.*U__DC.*(-(1/2).*I__ac.*(b__2d.*I__ac.*sin(abs(d))+2.*b__1d).*cos(abs(d))+...
