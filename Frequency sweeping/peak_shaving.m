...
Psp=yyP_bat*100; %% in W
Qsp=yyQ_bat*100; %% in W
...
eta_batt_2LC(:, find(eta_batt_2LC(2,:)>=99.999 )) = [];  %columns
eta_batt_TTYPE(:,find(eta_batt_TTYPE(2,:)>=99.999 )) = [];  %columns
eta_batt_NPCC(:,find(eta_batt_NPCC(2,:)>=99.999 ))= [];  %columns
eta_batt_ANPCC(:,find(eta_batt_ANPCC(2,:)>=99.999 )) = [];  %columns
...
figure(figindx)
plot(timerange3,Losses_2LC,timerange3,Losses_TTYPE,timerange3,Losses_NPCC,timerange3, Losses_ANPCC)
title('Converter losses')
legend('ANPCC','NPCC','TTYPE','2LC')
figindx=figindx+1;
