function [Pc_comp,Ps_comp,Ptot_comp,Ptot,I_DC]=npcc_comp(U__DC,Vac,Sac,ddeg,f__s)
...
I_elec_rms=(1/4)*I__ac*sin(d)*m*sqrt(2);

I_film_rms=sqrt(I__ac^2*(11*m^2*cos(d)^2*pi-2*m^2*Pi+24*sqrt(3)*cos(d)^2-...
72*m*cos(d)^2+8*Pi-12*sqrt(3))/(16*pi));

I_hf_rms=sqrt(I__ac^2*(9*m^2*cos(d)^2*Pi+24*sqrt(3)*cos(d)^2-...
72*m*cos(d)^2+8*pi-12*sqrt(3))/(16*pi));

I_DC=[I_dc_avg I_elec_rms I_film_rms I_hf_rms];
end